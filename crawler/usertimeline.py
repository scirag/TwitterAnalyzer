...
class TwitterSearchImpl(TwitterSearch):
    ...
    def __init__(self, rate_delay, error_delay, max_tweets):
        ...
        self.tweet_collection = db.col_usertimeline
        self.tweet_collection.delete_many({})
    ...
    def save_tweets(self, tweets):
        for tweet in tweets:
            ...
            if tweet['created_at'] is not None:
                ...
                tweet['timestamp'] = t
                fmt = "%Y-%m-%d %H:%M:%S"
                log.info("%i {%s} [%s] - %s" % (self.counter, tweet['tweet_id'], t.strftime(fmt), tweet['text']))
                self.tweet_collection.insert_one(tweet)
...
class TwitterSlicer(TwitterSearch):
    ...
    def save_tweets(self, tweets):
        ...
        :return: True always
        """
        log_filename = "06melihgokcek_t16.log"
        if self.counter == 0:
        ...
        for tweet in tweets:
            ...
            if tweet['created_at'] is not None:
                ...
                with open(log_filename, "at") as f:
                    f.write("%i {%s} [%s] - %s\n" % (self.counter, tweet['tweet_id'], t.strftime(fmt), tweet['text']))
...
def crawl():
    ...
    search_query = USERTIMELINE_QUERY
    ...
    """
    select_tweets_since = datetime.datetime.strptime("2013-08-02", '%Y-%m-%d')
    select_tweets_until = datetime.datetime.strptime("2017-02-21", '%Y-%m-%d')
    threads = 16
